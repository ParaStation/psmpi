...
int *get_group_ranks(MPIR_Comm * comm_ptr, MPIR_Group * group_ptr)
{
    ...
    mpi_errno = MPIR_Group_translate_ranks_impl(group_ptr, grp_sz, arr123, win_grp_ptr, res);
    if (mpi_errno)
        goto fn_fail;
...
int MPID_Win_fence(int assert, MPIR_Win * win_ptr)
{
    ...
    while ((win_ptr->rma_target_total_received != total_rma_puts_accs) ||
           win_ptr->rma_local_pending_cnt > win_ptr->rma_local_complete_cnt) {
...
int MPID_Win_complete(MPIR_Win * win_ptr)
{
    ...
    /* Wait for all local rma operations to be completed */
...
int MPID_Win_wait(MPIR_Win * win_ptr)
{
    ...
    for (i = 0; i < ranks_sz; i++) {
        ...
        while (win_ptr->rma_source_rank_received[rank] < win_ptr->rma_source_rank_expected[rank]) {
            MPID_PSP_LOCKFREE_CALL(pscom_wait_any());
        }
    }
...
static inline int do_trylock(MPIR_Win * win_ptr, int exclusive)
{
    if (!win_ptr->lock_cnt || (!win_ptr->lock_exclusive && !exclusive)) {
...
void MPID_do_recv_rma_unlock_req(pscom_request_t * req)
{
    ...
    if (win_ptr->rma_source_rank_expected[src_rank] < xhead_lock->rma_op_counter)
        win_ptr->rma_source_rank_expected[src_rank] = xhead_lock->rma_op_counter;
...
int MPID_Win_unlock(int dest, MPIR_Win * win_ptr)
{
    ...
    if (win_ptr->epoch_state != MPID_PSP_EPOCH_LOCK &&
...
int MPIDI_PSP_Win_wait_local_completion(int rank, MPIR_Win * win_ptr)
{
    ...
    while (win_ptr->rma_local_pending_rank[rank] > win_ptr->rma_local_complete_rank[rank]) {
        MPID_PSP_LOCKFREE_CALL(pscom_wait_any());
    }
