...
int MPIR_Iscatter_cdesc(CFI_cdesc_t* x0, int x1, MPI_Datatype x2, CFI_cdesc_t* x3, int x4, MPI_Datatype x5, int x6, MPI_Comm x7, MPI_Request * x8)
{
    int err = MPI_SUCCESS;
    ...
    if (buf3 == &MPIR_F08_MPI_BOTTOM) {
        buf3 = MPI_BOTTOM;
    } else if (buf3 == &MPIR_F08_MPI_IN_PLACE) {
        buf3 = MPI_IN_PLACE;
    }
    ...
    err = MPI_Iscatter(buf0, count0, dtype0, buf3, count3, dtype3, x6, x7, x8);
