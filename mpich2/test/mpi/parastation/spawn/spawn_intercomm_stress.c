    ...
    if ((world_rank == 0) && (parent_comm == MPI_COMM_NULL)) {
        ...
        for (j = 0; j < NUM_ROUNDS; j++) {
            ...
            for (i = 1; i < split_size; i++) {
                ...
                MPI_Irecv(rbuf, 128, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, split_comm,
                          &requests[msg_count++]);
            }
            ...
            for (i = 0; i < split_remote_size; i++) {
                ...
                MPI_Irecv(rbuf, 128, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, inter_comm,
                          &requests[msg_count++]);
            }
            ...
            for (i = 0; i < world_size; i++) {
                ...
                MPI_Irecv(rbuf, 128, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, spawn_comm,
                          &requests[msg_count++]);
            }
        }
