...
int main(int argc, char **argv)
{
    ...
    if (rank == 0)
        MTestPrintfMsg(1,
                       "*** Checking MPI_Iscatter for messages > ~%d MB with a datatype of > ~%ld MB\n",
    ...
    if (rank == 0) {
        MPI_Iscatter(buffer, FACTOR, contig_type, MPI_IN_PLACE, FACTOR, contig_type, 0,
                     MPI_COMM_WORLD, &request);
    } else {
        MPI_Iscatter(buffer, FACTOR, contig_type, buffer, FACTOR, contig_type, 0, MPI_COMM_WORLD,
                     &request);
    }
    ...
    if (rank != 0) {
        for (i = 0; i < SIZE; i++) {
            if (buffer[i] != 42 + rank) {
                if (errs < 10)
                    fprintf(stderr, "(%d) MPI_Iscatter: Error at position %zu: %d vs. %d\n", rank,
                            i, buffer[i], 42 + rank);
                errs++;
            }
        }
    ...
    if (rank == 0)
        MTestPrintfMsg(1,
                       "*** Checking MPI_Iscatterv for messages > ~%d MB with a datatype of > ~%ld MB\n",
                       (SIZE * sizeof(int)) / (1024 * 1024), extent / (1024 * 1024));
    ...
    if (rank == 0) {
        ...
        MPI_Iscatterv(buffer, sendcnts, displs, contig_type, MPI_IN_PLACE, FACTOR, contig_type, 0,
                      MPI_COMM_WORLD, &request);
    } else {
        MPI_Iscatterv(NULL, NULL, NULL, contig_type, buffer, FACTOR, contig_type, 0, MPI_COMM_WORLD,
                      &request);
    }
    ...
    if (rank != 0) {
        for (i = 0; i < SIZE; i++) {
            if (buffer[i] != 19 + rank) {
                if (errs < 10)
                    fprintf(stderr, "(%d) MPI_Iscatterv: Error at position %zu: %d vs. %d\n", rank,
                            i, buffer[i], 19 + rank);
                errs++;
            }
        }
