...
int main(int argc, char **argv)
{
    ...
    if (rank == 0)
        MTestPrintfMsg(1,
                       "*** Checking MPI_Send/Irecv for messages > ~%d MB with a datatype of > ~%ld MB\n",
    ...
    if (rank == 0)
        MTestPrintfMsg(1,
                       "*** Checking MPI_Get for messages > ~%d MB with a datatype of > ~%ld MB\n",
                       (SIZE * sizeof(int)) / (1024 * 1024), extent / (1024 * 1024));
    ...
    if (rank == 0)
        MTestPrintfMsg(1,
                       "*** Checking MPI_Put for messages > ~%d MB with a datatype of > ~%ld MB\n",
                       (SIZE * sizeof(int)) / (1024 * 1024), extent / (1024 * 1024));
    ...
    if (rank == 0)
        MTestPrintfMsg(1,
                       "*** Checking MPI_Accumulate for messages > ~%d MB with a datatype of > ~%ld MB\n",
                       (SIZE * sizeof(int)) / (1024 * 1024), extent / (1024 * 1024));
