...
int main(int argc, char** argv)
{
	...
	MPI_Type_vector(2, SIZE / (2 * factor) /* -> */ - 1 /* <- non-contiguous */, SIZE / (2 * factor), MPI_INT, &non_contig_type);
	MPI_Type_commit(&non_contig_type);
        ...
        if (rank == 0) MTestPrintfMsg(1, "*** Checking MPI_Send/Irecv with non-contiguous messages > ~%d MB\n", (SIZE * sizeof(int)) / (1024*1024));
        ...
        for (i = 0; i < SIZE; i++) {
                if ((rank == 0) || ((i < SIZE - factor) && ((i % (SIZE / (2 * factor) - 1)) - i / (SIZE / factor)))) {
                        if ((buffer[i] != 42)) {
                                if (errs < 10) fprintf(stderr, "(%d) MPI_Irecv: Error at position %d: %d vs. %d\n", rank, i, buffer[i], 42);
                                errs++;
                        }
                }
        }
	...
	if (rank == 0) MTestPrintfMsg(1, "*** Checking MPI_Get with non-contiguous messages > ~%d MB\n", (SIZE * sizeof(int)) / (1024*1024));
	...
	for (i = 0; i < SIZE; i++) {
		if ((rank == 0) || ((i < SIZE - factor) && ((i % (SIZE / (2 * factor) - 1)) - i / (SIZE / factor)))) {
			if (buffer[i] != 19) {
				if (errs < 10) fprintf(stderr ,"(%d) MPI_Get: Error at position %d: %d vs. %d\n", rank, i, buffer[i], 19);
				errs++;
			}
		}
	}
	...
	if (rank == 0) MTestPrintfMsg(1, "*** Checking MPI_Put with non-contiguous messages > ~%d MB\n", (SIZE * sizeof(int)) / (1024*1024));
	...
	for (i = 0; i < SIZE; i++) {
		if ((rank == 0) || ((i < SIZE - factor) && ((i % (SIZE / (2 * factor) - 1)) - i / (SIZE / factor)))) {
			if (buffer[i] != 23) {
				if (errs < 10) fprintf(stderr, "(%d) MPI_Put: Error at position %d: %d vs. %d\n", rank, i, buffer[i], 23);
				errs++;
			}
		}
	}
	...
	if (rank == 0) MTestPrintfMsg(1, "*** Checking MPI_Bcast with non-contiguous messages > ~%d MB\n", (SIZE * sizeof(int)) / (1024*1024));

	if (rank == 0) {
		for (i = 0; i < SIZE; i++) buffer[i] = 137;
	} else {
		for (i = 0; i < SIZE; i++) buffer[i] = 0;
	}
	...
	for (i = 0; i < SIZE; i++) {
		if ((rank == 0) || ((i < SIZE - factor) && ((i % (SIZE / (2 * factor) - 1)) - i / (SIZE / factor)))) {
			if ((buffer[i] != 137)) {
				if (errs < 10) fprintf(stderr, "(%d) MPI_Bcast: Error at position %zu: %d vs. %d\n", rank, i, buffer[i], 137);
				errs++;
			}
		}
	}
        ...
        if (rank == 0) MTestPrintfMsg(1, "*** Checking MPI_Ibcast with non-contiguous messages > ~%d MB\n", (SIZE * sizeof(int)) / (1024*1024));

        if (rank == 0) {
                for (i = 0; i < SIZE; i++) buffer[i] = 73;
        } else {
                for (i = 0; i < SIZE; i++) buffer[i] = 0;
        }
        ...
        for (i = 0; i < SIZE; i++) {
                if ((rank == 0) || ((i < SIZE - factor) && ((i % (SIZE / (2 * factor) - 1)) - i / (SIZE / factor)))) {
                        if ((buffer[i] != 73)) {
                                if (errs < 10) fprintf(stderr, "(%d) MPI_Ibcast: Error at position %zu: %d vs. %d\n", rank, i, buffer[i], 73);
                                errs++;
                        }
                }
        }
