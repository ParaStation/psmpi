...
int main(int argc, char** argv)
{
	...
	MPI_Ialltoall(MPI_IN_PLACE, FACTOR, contig_type, buffer, FACTOR, contig_type, MPI_COMM_WORLD, &request);
	...
	MPI_Ialltoallv(MPI_IN_PLACE, NULL, NULL, contig_type, buffer, recvcnts, rdispls, contig_type, MPI_COMM_WORLD, &request);

	MPI_Wait(&request, MPI_STATUS_IGNORE);

	for (i = 0; i < nprocs * SIZE; i++) {
		if (buffer[i] != 19 + i / SIZE) {
			if (errs < 10) fprintf(stderr, "(%d) MPI_Ialltoallv: Error at position %zu: %d vs. %ld\n", rank, i, buffer[i], 19 + i / SIZE);
			errs++;
		}
	}
