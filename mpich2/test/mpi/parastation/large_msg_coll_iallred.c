...
int main(int argc, char** argv)
{
	...
	if (rank == 0) MTestPrintfMsg(1, "*** Checking MPI_Iallreduce for messages > ~%d MB\n", (SIZE * sizeof(int)) / (1024*1024));

	for (i = 0; i < SIZE; i++) buffer[i] = 42;

	MPI_Iallreduce(MPI_IN_PLACE, buffer, SIZE, MPI_INT, MPI_SUM, MPI_COMM_WORLD, &request);

	MPI_Wait(&request, MPI_STATUS_IGNORE);

	for (i = 0; i < SIZE; i++) {
		if (buffer[i] != 42 * nprocs) {
			if (errs < 10) fprintf(stderr, "(%d) MPI_Iallreduce: Error at position %zu: %d vs. %d\n", rank, i, buffer[i], 42 * nprocs);
			errs++;
		}
	}
