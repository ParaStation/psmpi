...
int main(int argc, char *argv[])
{
	...
	if (parent_comm == MPI_COMM_NULL) {
		MPI_Comm_spawn((char*)"./spawn_univ_anysrc_order_mixed_comms", MPI_ARGV_NULL, 2, MPI_INFO_NULL, 0, MPI_COMM_SELF, &spawn_comm, errcodes);
	} else {
	...
	if (univ_rank == 0) {
		...
		MPI_Irecv(&univ_msg_recv_buf[univ_msg_recv_cnt++], 1, MPI_INT, MPI_ANY_SOURCE, 0, univ_comm, &request[request_cnt++]);

		MPI_Irecv(&univ_msg_recv_buf[univ_msg_recv_cnt++], 1, MPI_INT, univ_size - 1, 0, univ_comm, &request[request_cnt++]);
	...
	if (world_rank == 0) {
		...
		MPI_Irecv(&world_msg_recv_buf[world_msg_recv_cnt++], 1, MPI_INT, world_size - 1, 0, MPI_COMM_WORLD, &request[request_cnt++]);

		MPI_Irecv(&world_msg_recv_buf[world_msg_recv_cnt++], 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &request[request_cnt++]);
	...
	if (univ_rank == univ_size - 1) {
		...
		MPI_Isend(&univ_msg_send_buf[univ_msg_send_cnt++], 1, MPI_INT, 0, 0, univ_comm, &request[request_cnt++]);
	}

	if (world_rank == world_size - 1) {

		MPI_Isend(&world_msg_send_buf[world_msg_send_cnt++], 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &request[request_cnt++]);
