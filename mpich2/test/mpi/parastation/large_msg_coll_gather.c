...
int main(int argc, char **argv)
{
    ...
    if (rank == 0)
        MTestPrintfMsg(1,
                       "*** Checking MPI_Gather for messages > ~%d MB with a datatype of > ~%ld MB\n",
    ...
    if (rank == 0) {
        MPI_Gather(MPI_IN_PLACE, FACTOR, contig_type, buffer, FACTOR, contig_type, 0,
                   MPI_COMM_WORLD);
    } else {
    ...
    if (rank == 0) {
        for (i = SIZE; i < nprocs * SIZE; i++) {
            if (buffer[i] != 42 + i / SIZE) {
                if (errs < 10)
                    fprintf(stderr, "(%d) MPI_Gather: Error at position %zu: %d vs. %ld\n", rank, i,
                            buffer[i], 42 + i / SIZE);
                errs++;
            }
        }
    ...
    if (rank == 0)
        MTestPrintfMsg(1,
                       "*** Checking MPI_Gatherv for messages > ~%d MB with a datatype of > ~%ld MB\n",
                       (SIZE * sizeof(int)) / (1024 * 1024), extent / (1024 * 1024));
    ...
    if (rank == 0) {
        ...
        MPI_Gatherv(MPI_IN_PLACE, FACTOR, contig_type, buffer, recvcnts, displs, contig_type, 0,
                    MPI_COMM_WORLD);
    } else {
    ...
    if (rank == 0) {
        for (i = SIZE; i < nprocs * SIZE; i++) {
            if (buffer[i] != 19 + i / SIZE) {
                if (errs < 10)
                    fprintf(stderr, "(%d) MPI_Gatherv: Error at position %zu: %d vs. %ld\n", rank,
