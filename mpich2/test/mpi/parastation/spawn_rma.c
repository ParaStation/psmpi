...
int main( int argc, char *argv[] )
{
     ...
     if(parent_comm == MPI_COMM_NULL) {
	  ...
	  MPI_Comm_spawn((char*)"./spawn_rma", MPI_ARGV_NULL, world_size, MPI_INFO_NULL, 0, MPI_COMM_WORLD, &spawn_comm, errcodes);
	  free(errcodes);
     } else {
     ...
     for (cs=0; cs<4; cs++) {
	  ...
	  if(univ_rank == 0) {
	       MPI_Win_create(buf, sizeof(char)*n*univ_size, sizeof(char), MPI_INFO_NULL, univ_comm, &win);
	  } else {
