...
int main(int argc, char *argv[])
{
    ...
    if (parent_comm == MPI_COMM_NULL) {
        MPI_Comm_spawn((char *) "./spawn_univ_anysrc", MPI_ARGV_NULL, 2, MPI_INFO_NULL, 0,
    ...
    for (j = 0; j < 8; j++) {
        ...
        if (univ_rank == root) {
            ...
            for (i = 0; i < univ_size; i++) {
                ...
                if ((j % 4) == 3)
                    MPI_Probe(MPI_ANY_SOURCE, i, univ_comm, MPI_STATUS_IGNORE);

                MPI_Irecv(&remote_ranks[i], 1, MPI_INT, MPI_ANY_SOURCE, i, univ_comm,
                          &recv_reqs[i]);
            }
