...
int main(int argc, char** argv)
{
...
#if 1   // Checking MPI_Iallgather:

	if (rank == 0) MTestPrintfMsg(1, "*** Checking MPI_Iallgather for messages > ~%d MB with a datatype of > ~%ld MB\n", (SIZE * sizeof(int)) / (1024*1024), extent / (1024*1024));

	for (i = 0; i < nprocs * SIZE; i++) buffer[i] = 42 + rank;

	MPI_Iallgather(MPI_IN_PLACE, FACTOR, contig_type, buffer, FACTOR, contig_type, MPI_COMM_WORLD, &request);

        MPI_Wait(&request, MPI_STATUS_IGNORE);

	for (i = 0; i < nprocs * SIZE; i++) {
		if (buffer[i] != 42 + i / SIZE) {
			if (errs < 10) fprintf(stderr, "(%d) MPI_Iallgather: Error at position %zu: %d vs. %ld\n", rank, i, buffer[i], 42 + i / SIZE);
			errs++;
		}
	}
#endif

#if 1   // Checking MPI_Iallgatherv:

	MPI_Barrier(MPI_COMM_WORLD);

	if (rank == 0) MTestPrintfMsg(1, "*** Checking MPI_Iallgatherv for messages > ~%d MB with a datatype of > ~%ld MB\n", (SIZE * sizeof(int)) / (1024*1024), extent / (1024*1024));

	for (i = 0; i < nprocs * SIZE; i++) buffer[i] = 19 + rank;
	...
	MPI_Iallgatherv(MPI_IN_PLACE, FACTOR, contig_type, buffer, recvcnts, displs, contig_type, MPI_COMM_WORLD, &request);

        MPI_Wait(&request, MPI_STATUS_IGNORE);

	for (i = 0; i < nprocs * SIZE; i++) {
		if (buffer[i] != 19 + i / SIZE) {
			if (errs < 10) fprintf(stderr, "(%d) MPI_Iallgatherv: Error at position %zu: %d vs. %ld\n", rank, i, buffer[i], 19 + i / SIZE);
			errs++;
		}
	}
