...
/* If this test fails, then try to set MPIR_CONTEXT_SUBCOMM_WIDTH to (0) in mpich2/src/include/mpiimpl.h */
...
int main(int argc, char* argv[])
{
...
#if !defined(MPIX_MSA_AWARENESSNESS) || (MPIX_MSA_AWARENESSNESS == 0)
	...
	for(i=0; i<NUM_COMMS; i++) {
		...
		if(rc != MPI_SUCCESS) {
			...
			printf("Try to set MPIR_CONTEXT_SUBCOMM_WIDTH to (0) in mpich2/src/include/mpiimpl.h to get more contexts.\n");
			printf("This test is known to fail with CH3 device as well as if MPIDI_PSP_WITH_MSA_AWARENESSNESS is enabled.\n\n");
