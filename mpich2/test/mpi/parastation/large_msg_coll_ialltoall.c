...
int main(int argc, char** argv)
{
	...
	if (rank == 0) MTestPrintfMsg(1, "*** Checking MPI_Ialltoall for messages > ~%d MB with a datatype of > ~%ld MB\n", (SIZE * sizeof(int)) / (1024*1024), extent / (1024*1024));

	for (i = 0; i < nprocs * SIZE; i++) sbuffer[i] = 42 + rank ;

	MPI_Ialltoall(sbuffer, FACTOR, contig_type, rbuffer, FACTOR, contig_type, MPI_COMM_WORLD, &request);

	MPI_Wait(&request, MPI_STATUS_IGNORE);

	for (i = 0; i < nprocs * SIZE; i++) {
		if (rbuffer[i] != 42 + i / SIZE) {
			if (errs < 10) fprintf(stderr, "(%d) MPI_Ialltoall: Error at position %zu: %d vs. %ld\n", rank, i, rbuffer[i], 42 + i / SIZE);
			errs++;
		}
	}
#endif
	...
	if (rank == 0) MTestPrintfMsg(1, "*** Checking MPI_Ialltoallv for messages > ~%d MB with a datatype of > ~%ld MB\n", (SIZE * sizeof(int)) / (1024*1024), extent / (1024*1024));
	...
	for (i = 0; i < nprocs * SIZE; i++) sbuffer[i] = 19 + rank ;

	MPI_Ialltoallv(sbuffer, sendcnts, sdispls, contig_type, rbuffer, recvcnts, rdispls, contig_type, MPI_COMM_WORLD, &request);

	MPI_Wait(&request, MPI_STATUS_IGNORE);

	for (i = 0; i < nprocs * SIZE; i++) {
		if (rbuffer[i] != 19 + i / SIZE) {
			if (errs < 10) fprintf(stderr, "(%d) MPI_Ialtoallv: Error at position %zu: %d vs. %ld\n", rank, i, rbuffer[i], 19 + i / SIZE);
			errs++;
		}
	}
