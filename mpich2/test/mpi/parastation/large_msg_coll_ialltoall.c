...
int main(int argc, char **argv)
{
    ...
    MPI_Ialltoall(sbuffer, FACTOR, contig_type, rbuffer, FACTOR, contig_type, MPI_COMM_WORLD,
                  &request);
    ...
    MPI_Ialltoallv(sbuffer, sendcnts, sdispls, contig_type, rbuffer, recvcnts, rdispls, contig_type,
    ...
    for (i = 0; i < nprocs * SIZE; i++) {
        if (rbuffer[i] != 19 + i / SIZE) {
            if (errs < 10)
                fprintf(stderr, "(%d) MPI_Ialtoallv: Error at position %zu: %d vs. %ld\n", rank, i,
                        rbuffer[i], 19 + i / SIZE);
            errs++;
        }
    }
