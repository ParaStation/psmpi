...
int main(int argc, char** argv)
{
	...
	if (rank == 0) MTestPrintfMsg(1, "*** Checking MPI_Allgather for messages > ~%d MB with a datatype of > ~%ld MB\n", (SIZE * sizeof(int)) / (1024*1024), extent / (1024*1024));
	...
	for (i = 0; i < nprocs * SIZE; i++) {
		if (buffer[i] != 42 + i / SIZE) {
			if (errs < 10) fprintf(stderr, "(%d) MPI_Allgather: Error at position %zu: %d vs. %ld\n", rank, i, buffer[i], 42 + i / SIZE);
			errs++;
		}
	}
#endif
	...
	if (rank == 0) MTestPrintfMsg(1, "*** Checking MPI_Allgatherv for messages > ~%d MB with a datatype of > ~%ld MB\n", (SIZE * sizeof(int)) / (1024*1024), extent / (1024*1024));
	...
	MPI_Allgatherv(MPI_IN_PLACE, FACTOR, contig_type, buffer, recvcnts, displs, contig_type, MPI_COMM_WORLD);

	for (i = 0; i < nprocs * SIZE; i++) {
		if (buffer[i] != 19 + i / SIZE) {
			if (errs < 10) fprintf(stderr, "(%d) MPI_Allgatherv: Error at position %zu: %d vs. %ld\n", rank, i, buffer[i], 19 + i / SIZE);
			errs++;
		}
	}
