...
int main(int argc, char** argv)
{
	...
	// Checking MPI_Bcast for large messages:

	if (rank == 0) MTestPrintfMsg(1, "*** Checking MPI_Bcast for messages > ~%d MB with a datatype of > ~%ld MB\n", (SIZE * sizeof(int)) / (1024*1024), extent / (1024*1024));
	...
	MPI_Bcast(buffer, FACTOR, contig_type, 0, MPI_COMM_WORLD);
