...
int main(int argc, char** argv)
{
...
#if 1   // Checking with MPI_Scatter:

	if (rank == 0) MTestPrintfMsg(1, "*** Checking MPI_Scatter for messages > ~%d MB with a datatype of > ~%ld MB\n", (SIZE * sizeof(int)) / (1024*1024), extent / (1024*1024));
	...
	if (rank == 0) {
		MPI_Scatter(buffer, FACTOR, contig_type, MPI_IN_PLACE, FACTOR, contig_type, 0, MPI_COMM_WORLD);
	} else {
		MPI_Scatter(buffer, FACTOR, contig_type, buffer, FACTOR, contig_type, 0, MPI_COMM_WORLD);
	}

	if (rank != 0) {
		for (i = 0; i < SIZE; i++) {
			if (buffer[i] != 42 + rank) {
				if (errs < 10) fprintf(stderr, "(%d) MPI_Scatter: Error at position %zu: %d vs. %d\n", rank, i, buffer[i], 42 + rank);
				errs++;
			}
		}
	}
...
#if 1   // Checking with MPI_Scatterv:

	MPI_Barrier(MPI_COMM_WORLD);

	if (rank == 0) MTestPrintfMsg(1, "*** Checking MPI_Scatterv for messages > ~%d MB with a datatype of > ~%ld MB\n", (SIZE * sizeof(int)) / (1024*1024), extent / (1024*1024));
	...
	if (rank == 0) {
		...
		MPI_Scatterv(buffer, sendcnts, displs, contig_type, MPI_IN_PLACE, FACTOR, contig_type, 0, MPI_COMM_WORLD);
	} else {
		MPI_Scatterv(NULL, NULL, NULL, contig_type, buffer, FACTOR, contig_type, 0, MPI_COMM_WORLD);
	}

	if (rank != 0) {
		for (i = 0; i < SIZE; i++) {
			if (buffer[i] != 19 + rank) {
				if (errs < 10) fprintf(stderr, "(%d) MPI_Scatterv: Error at position %zu: %d vs. %d\n", rank, i, buffer[i], 19 + rank);
				errs++;
			}
		}
		free(displs);
