...
int main(int argc, char** argv)
{
	...
	if (rank == 0) MTestPrintfMsg(1, "*** Checking MPI_Alltoall for messages > ~%d MB with a datatype of > ~%ld MB\n", (SIZE * sizeof(int)) / (1024*1024), extent / (1024*1024));

	for (i = 0; i < nprocs * SIZE; i++) buffer[i] = 42 + rank ;

	MPI_Alltoall(MPI_IN_PLACE, FACTOR, contig_type, buffer, FACTOR, contig_type, MPI_COMM_WORLD);

	for (i = 0; i < nprocs * SIZE; i++) {
		if (buffer[i] != 42 + i / SIZE) {
			if (errs < 10) fprintf(stderr, "(%d) MPI_Alltoall: Error at position %zu: %d vs. %ld\n", rank, i, buffer[i], 42 + i / SIZE);
			errs++;
		}
	}
#endif
	...
	if (rank == 0) MTestPrintfMsg(1, "*** Checking MPI_Alltoallv for messages > ~%d MB with a datatype of > ~%ld MB\n", (SIZE * sizeof(int)) / (1024*1024), extent / (1024*1024));
	...
	MPI_Alltoallv(MPI_IN_PLACE, NULL, NULL, contig_type, buffer, recvcnts, rdispls, contig_type, MPI_COMM_WORLD);

	for (i = 0; i < nprocs * SIZE; i++) {
		if (buffer[i] != 19 + i / SIZE) {
			if (errs < 10) fprintf(stderr, "(%d) MPI_Alltoallv: Error at position %zu: %d vs. %ld\n", rank, i, buffer[i], 19 + i / SIZE);
			errs++;
		}
	}
