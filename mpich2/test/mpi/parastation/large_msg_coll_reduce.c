...
int main(int argc, char** argv)
{
	...
	if (rank == 0) MTestPrintfMsg(1, "*** Checking MPI_Reduce for messages > ~%d MB\n", (SIZE * sizeof(int)) / (1024*1024));

	for (i = 0; i < SIZE; i++) buffer[i] = 42;

	if (rank == 0) {
		MPI_Reduce(MPI_IN_PLACE, buffer, SIZE, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
	} else {
		MPI_Reduce(buffer, buffer, SIZE, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
	}

	if (rank == 0) {
		for (i = 0; i < SIZE; i++) {
			if (buffer[i] != 42 * nprocs) {
				if (errs < 10) fprintf(stderr, "(%d) MPI_Reduce: Error at position %zu: %d vs. %d\n", rank, i, buffer[i], 42 * nprocs);
				errs++;
			}
		}
	}
