...
int main(int argc, char *argv[])
{
    ...
    if (rbuf[NUM_ELEMENTS - 1] != RESULT(size)) {
        printf
            ("Rank %d - Allreduce Error: [rbuf[NUM_ELEMENTS-1] = %d] != [size*(size-1)/2) = %d]\n",
    ...
    if (rank == 0) {
        if (rbuf[0] != RESULT(size)) {
            printf
                ("Rank 0 - Reduce_scatter_block Error: [rbuf[0] = %d] != [size*(size-1)/2) = %d]\n",
                 rbuf[0], RESULT(size));
            ++errs;
        }
    }
    ...
    if (rank == size - 1) {
        if (rbuf[NUM_ELEMENTS / size - 1] != RESULT(size)) {
            printf
                ("Rank %d - Reduce_scatter_block Error: [rbuf[NUM_ELEMENTS/size-1] = %d] != [size*(size-1)/2) = %d]\n",
    ...
    for (i = 0; i < size; i++)
        recvcounts[i] = NUM_ELEMENTS / size;
    ...
    if (rank == size - 1) {
        if (rbuf[NUM_ELEMENTS / size - 1] != RESULT(size)) {
            printf
                ("Rank %d - Reduce_scatter Error: [rbuf[NUM_ELEMENTS/size-1] = %d] != [size*(size-1)/2) = %d]\n",
                 rank, rbuf[NUM_ELEMENTS / size - 1], RESULT(size));
            ++errs;
        }
    }
    ...
    if (rbuf[NUM_ELEMENTS - 1] != RESULT(size)) {
        printf
            ("Rank %d - Allreduce IN_PLACE Error: [rbuf[NUM_ELEMENTS-1] = %d] != [size*(size-1)/2 = %d]\n",
             rank, rbuf[NUM_ELEMENTS - 1], RESULT(size));
        ++errs;
    }
    ...
    init_buffers(rbuf, rbuf, crbuf, crbuf, len);
    MPI_Reduce_scatter_block(MPI_IN_PLACE, crbuf, NUM_ELEMENTS / size, MPI_INT, MPI_SUM,
                             MPI_COMM_WORLD);
    CUDA_CHECK(cudaMemcpy(rbuf, crbuf, len / size, cudaMemcpyDeviceToHost));
    ...
    if (rank == 0) {
        if (rbuf[0] != RESULT(size)) {
            printf
                ("Rank 0 - Reduce_scatter_block IN_PLACE Error: [rbuf[0] = %d] != [size*(size-1)/2 = %d]\n",
                 rbuf[0], RESULT(size));
            ++errs;
        }
    }
    ...
    if (rank == size - 1) {
        ...
        if (val1 != val2) {
            printf
                ("Rank %d - Reduce_scatter_block IN_PLACE Error: [rbuf[NUM_ELEMENTS/size-1] = %d] != [size*(size-1)/2 = %d]\n",
                 rank, val1, val2);
            ++errs;
        }
    }
    ...
    if (rank == 0) {
        if (rbuf[0] != RESULT(size)) {
            printf
                ("Rank 0 - Reduce_scatter IN_PLACE Error: [rbuf[0] = %d] != [size*(size-1)/2 = %d]\n",
                 rbuf[0], RESULT(size));
            ++errs;
        }
    }
    ...
    if (rank == size - 1) {
        ...
        if (val1 != val2) {
            printf
                ("Rank %d - Reduce_scatter IN_PLACE Error: [rbuf[NUM_ELEMENTS/size-1] = %d] != [size*(size-1)/2 = %d]\n",
                 rank, val1, val2);
            ++errs;
        }
    }
