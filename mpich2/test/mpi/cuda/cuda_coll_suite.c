...
int main(int argc, char *argv[])
{
    ...
    init_buffers(sbuf, rbuf, csbuf, crbuf, len);
    MPI_Scatter(csbuf, NUM_ELEMENTS / size, MPI_INT, crbuf, NUM_ELEMENTS / size, MPI_INT, 0,
                MPI_COMM_WORLD);
    CUDA_CHECK(cudaMemcpy(rbuf, crbuf, len, cudaMemcpyDeviceToHost));
    ...
    init_buffers(sbuf, rbuf, csbuf, crbuf, len);
    MPI_Gather(csbuf, NUM_ELEMENTS / size, MPI_INT, crbuf, NUM_ELEMENTS / size, MPI_INT, 0,
               MPI_COMM_WORLD);
    CUDA_CHECK(cudaMemcpy(rbuf, crbuf, len, cudaMemcpyDeviceToHost));
    ...
    if (rank == 0) {
        for (i = 0; i < NUM_ELEMENTS; i++) {
            ...
            if (val1 != val2) {
                printf
                    ("Gather Error: [rbuf[i] = %d] != [(i % (NUM_ELEMENTS/size)) + (i / (NUM_ELEMENTS/size)) = %d]\n",
                     val1, val2);
                errs++;
            }
        }
    ...
    init_buffers(sbuf, rbuf, csbuf, crbuf, len);
    MPI_Allgather(csbuf, NUM_ELEMENTS / size, MPI_INT, crbuf, NUM_ELEMENTS / size, MPI_INT,
                  MPI_COMM_WORLD);
    CUDA_CHECK(cudaMemcpy(rbuf, crbuf, len, cudaMemcpyDeviceToHost));
    ...
    for (i = 0; i < NUM_ELEMENTS; i++) {
        ...
        if (val1 != val2) {
            printf
                ("Allgather Error: [rbuf[i] = %d] != [(i % (NUM_ELEMENTS/size)) + (i / (NUM_ELEMENTS/size)) = %d]\n",
                 val1, val2);
            errs++;
        }
    }
    ...
    init_buffers(sbuf, rbuf, csbuf, crbuf, len);
    MPI_Alltoall(csbuf, NUM_ELEMENTS / size, MPI_INT, crbuf, NUM_ELEMENTS / size, MPI_INT,
                 MPI_COMM_WORLD);
    CUDA_CHECK(cudaMemcpy(rbuf, crbuf, len, cudaMemcpyDeviceToHost));
    ...
    for (i = 0; i < NUM_ELEMENTS; i++) {
        int val1 = rbuf[i];
        int val2 =
            (i % (NUM_ELEMENTS / size)) + (i / (NUM_ELEMENTS / size)) +
            rank * (NUM_ELEMENTS / size);

        if (val1 != val2) {
            printf
                ("Alltoall Error: [rbuf[i] = %d] != [(i % (NUM_ELEMENTS/size)) + (i / (NUM_ELEMENTS/size)) + rank * (NUM_ELEMENTS/size) = %d]\n",
                 val1, val2);
            errs++;
        }
    }
