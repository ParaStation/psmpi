...
int main(int argc, char *argv[])
{
	...
	MPI_Scan(cinmat_blob, coutmat_blob, 1, column_t, column_add, MPI_COMM_WORLD);
	...
	for (i=0; i < NUM_ELEMENTS ; ++i) {
		/* check first column */
		if (fabs(outmat[i][0] - (rank + 1.0)) > MAX_ERR) {
	...
	/* perform the reduction */
	MPI_Exscan(cinmat_blob, coutmat_blob, 1, column_t, column_add, MPI_COMM_WORLD);
